...
@WebServlet(name = "selectOrdersByViewer", urlPatterns = "/librarian/selectOrdersByViewer")
public class OrderListByViewerServletLibrarian extends HttpServlet {
    private static final Logger log = Logger.getLogger(OrderListByViewerServletLibrarian.class);
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            String id = req.getParameter("viewerId");
            Connection connection = DatabaseFunctions.getConnection();
            OrdersService ordersService = new OrdersService(connection);
            List orders = ordersService.searchOrders("viewer_id = " + Integer.parseInt(id), "date_created");
            checkPenalty(orders, ordersService);
            req.getSession().setAttribute("orders", ordersService.searchOrders("viewer_id = " + Integer.parseInt(id), "date_created"));
